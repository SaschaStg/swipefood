...
To start the Application just add the following files in die backend folder:

- jwt.secret, which contains any secret for the jwt token

- spoonacularapi.key, which contains the api key for the spoonacular api
...
You can create a new account or login with the following credentials:
    ...
    - username: test !WIP! /issues/67
    - password: test !WIP! /issues/67
...
```
cd existing_repo
git remote add origin https://gitlab.mi.hdm-stuttgart.de/mwa/ss23/swipefood.git
git branch -M main
...
- [ ] [Set up project integrations](https://gitlab.mi.hdm-stuttgart.de/mwa/ss23/swipefood/-/settings/integrations)
